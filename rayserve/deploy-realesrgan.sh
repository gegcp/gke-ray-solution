...
# Deploy Real-ESRGAN GPU and TPU inference service to GKE
set -e
...
CONFIGMAP_NAME="realesrgan-apps"
RAYSERVICE_NAME="realesrgan-gpu-tpu"

echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}Real-ESRGAN RayService Deployment${NC}"
echo -e "${BLUE}============================================${NC}"
echo ""
...
if [ -z "$HEAD_POD" ]; then
    ...
    echo "GPU endpoint:"
    kubectl exec -n $NAMESPACE $HEAD_POD -- curl -s http://localhost:8000/gpu/health 2>/dev/null | python3 -m json.tool 2>/dev/null || echo "Not ready yet"

    echo -e "\nTPU endpoint:"
    kubectl exec -n $NAMESPACE $HEAD_POD -- curl -s http://localhost:8000/tpu/health 2>/dev/null | python3 -m json.tool 2>/dev/null || echo "Not ready yet"
fi
...
echo "   ./test_realesrgan_apps.py"
echo ""
echo "5. View logs:"
echo "   kubectl logs -n $NAMESPACE $HEAD_POD --tail=100"
echo ""
echo -e "${BLUE}API Endpoints:${NC}"
echo "  GPU: http://localhost:8000/gpu/upscale"
echo "  TPU: http://localhost:8000/tpu/upscale"
echo ""
echo -e "${YELLOW}Note: If weights are not loaded, the service will use random initialization.${NC}"
echo -e "${YELLOW}To use pre-trained weights, update WEIGHT_PATH in the YAML file.${NC}"
echo ""
